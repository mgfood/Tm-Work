...
def print_header():
    ...
    ip = get_ipv4()
    print(f"{C.B}╔════════════════════════════════════════════════════════╗{C.E}")
    print(f"{C.B}║{C.E} 🚀 {C.BOLD}{C.H}        Центр управления TmWork (LINUX v4.0-RU){C.E}     {C.B}║{C.E}")
    print(f"{C.B}╠════════════════════════════════════════════════════════╣{C.E}")
...
def start_postgres():
    ...
    choice = input(f"{C.Y}[?] Запустить базу данных через systemctl? (y/n): {C.E}").lower()
    if choice == 'y':
        print(f"{C.CY}[*] Выполняю 'sudo systemctl start postgresql'...{C.E}")
...
def kill_processes():
    ...
    print(f"\n{C.Y}🛑 Останавливаю все активные сервисы...{C.E}")
...
def launch_services(mode):
    ...
    try:
        ...
        if mode == "silent":
            ...
            # Режим отдельных окон
            be_win_cmd = f'x-terminal-emulator -T "ЛОГИ БЭКЕНДА" -e "bash -c \'{VENV_PYTHON} manage.py runserver 0.0.0.0:8000; exec bash\'"'
            fe_win_cmd = f'x-terminal-emulator -T "ЛОГИ ФРОНТЕНДА" -e "bash -c \'npm run dev; exec bash\'"'
            
            subprocess.Popen(be_win_cmd, shell=True, cwd=BACKEND_DIR)
            subprocess.Popen(fe_win_cmd, shell=True, cwd=FRONTEND_DIR)
            
            # Заглушки для отслеживания состояния
        ...
        print(f"{C.R}❌ Ошибка запуска: {e}{C.E}")
        kill_processes()
...
def main():
    ...
    try:
        while True:
            ...
            print(f" {C.BOLD}[4]{C.E} 🏗️  Выполнить миграции")
            print(f" {C.BOLD}[5]{C.E} 🔑 Создать суперпользователя")
            print(f" {C.BOLD}[6]{C.E} 🧹 Очистить и сбросить БД (Выбор)")
            print(f" {C.BOLD}[7]{C.E} 🌍 Проверить локализацию")
            print(f" {C.BOLD}[8]{C.E} 📦 Установить зависимости")
            print(f"{C.B}──────────────────────────────────────────────────────────{C.E}")
            ...
            if choice in ['1', '1']: launch_services("silent")
            elif choice == '2': launch_services("window")
            elif choice == '3': launch_services("file")
            elif choice == 'X' or choice == 'Х': 
                kill_processes()
                time.sleep(1)
            elif choice == '4' or choice == '4': run_tool("Миграции", "🏗️", f'"{VENV_PYTHON}" manage.py migrate', BACKEND_DIR)
            elif choice == '5': run_tool("Админ", "🔑", f'"{VENV_PYTHON}" manage.py createsuperuser', BACKEND_DIR)
            elif choice == '6':
                db_reset_menu()
            elif choice == '7': run_tool("Локализация", "🌍", f'"{VENV_PYTHON}" {ROOT_DIR}/scripts/locales_check.py', BACKEND_DIR)
            elif choice == '8':
                ...
                print(f"{C.R}❌ Некорректный ввод.{C.E}")
                time.sleep(0.5)
    finally:
        sys.stdout.write(C.SHOW_CURSOR + C.EXIT_ALT)
        sys.stdout.flush()
