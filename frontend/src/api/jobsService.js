...
const jobsService = {
    ...
    getMyJobs: async (userId) => {
        // Pass client param to see drafts too
        const response = await apiClient.get('/jobs/', { params: { client: userId } });
        return response.data;
    },

    getMyWorks: async (userId) => {
        const response = await apiClient.get('/jobs/', { params: { freelancer: userId } });
        return response.data;
    },
