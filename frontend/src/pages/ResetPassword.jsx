...
const ResetPassword = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            showToast(t('resetPassword.success_msg'), 'success');
            navigate('/login');
        } catch (err) {
            ...
            if (typeof errorData === 'object') {
                ...
                setError(t('resetPassword.error_generic'));
            }
        } finally {
        ...
        <div className="flex-grow flex items-center justify-center px-6 py-12 bg-slate-50">
            <div className="w-full max-w-md">
                <Link to="/forgot-password" className="inline-flex items-center gap-2 text-slate-500 hover:text-primary-600 mb-6 transition-colors group">
                    ...
                    <span className="font-medium">{t('resetPassword.back_to_request')}</span>
                </Link>
                ...
                <div className="premium-card p-10 animate-in fade-in zoom-in duration-500">
                    <div className="text-center mb-8">
                        ...
                        <h2 className="text-3xl font-bold text-slate-900">{t('resetPassword.title')}</h2>
                        <p className="text-slate-500 mt-2">{t('resetPassword.subtitle')}</p>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-5">
                        <div className="space-y-1">
                            <label className="block text-sm font-semibold text-slate-700">{t('resetPassword.label_email')}</label>
                            <input
                                type="email"
                                name="email"
                                required
                        ...
                        <div className="space-y-1">
                            <label className="block text-sm font-semibold text-slate-700">{t('resetPassword.label_code')}</label>
                            <div className="relative">
                                <input
                                    ...
                                    placeholder={t('resetPassword.placeholder_code')}
                                    value={formData.code}
                                    onChange={handleChange}
                                />
                        ...
                        <div className="space-y-1">
                            <label className="block text-sm font-semibold text-slate-700">{t('resetPassword.label_new_password')}</label>
                            <div className="relative">
                                <input
                                    type="password"
                                    name="password"
                        ...
                        <div className="space-y-1">
                            <label className="block text-sm font-semibold text-slate-700">{t('resetPassword.label_confirm_password')}</label>
                            <div className="relative">
                                <input
                                    type="password"
                        ...
                        >
                            {isSubmitting ? (
                                ...
                                <>
                                    <span>{t('resetPassword.btn_submit')}</span>
                                    <ShieldCheck size={18} />
                                </>
                            )}
                        </button>
